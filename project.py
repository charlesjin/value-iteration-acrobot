...
try:
    assert not retrain
    loaded = np.load(f"outputs/precompute/transition_{resolution}.npy").reshape(shape)
    ...
    np.save(f"outputs/precompute/transition_{resolution}", transition_matrix)

try:
    assert not retrain
    ctg = np.load(f"outputs/precompute/ctg_{resolution}.npy")
    ...
    np.save(f"outputs/precompute/ctg_{resolution}", state_space.data[0])

# simulate
for idx, start_state in enumerate([(0, 0, 0, 0), (pi, 0, 0, 0), (pi / 2., 0, 0 ,0), (1, 0, 0, 0)]):
    sim(f"outputs/videos/output_{idx}", start_state, env, state_space, action_space)
    sim(f"outputs/videos/natural_{idx}", start_state, env)
